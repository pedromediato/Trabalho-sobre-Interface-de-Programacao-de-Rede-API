...
TEXTO_AJUDA = (
    ...
    "• /clear             -> Limpa o histórico da janela atual\n"
    "• /apagar <id>       -> Apaga uma mensagem pública pelo ID (ex: /apagar 3)\n"
...
class JanelaChatPrivado(ctk.CTkToplevel):
    ...
    def enviar_mensagem(self):
        ...
        # Envio Normal de Mensagem Privada
        hora_atual = datetime.now().strftime("%H:%M")
        self.exibir_mensagem(hora_atual, self.app_principal.apelido, texto)
        self.app_principal.enviar_msg_privada(self.destinatario, texto)
        self.ent_mensagem.delete(0, "end")

    def exibir_mensagem(self, hora, remetente, texto):
        self.area_chat.configure(state="normal")
        ...
        self.area_chat._textbox.insert("end", f"[{hora}] <{nome_exibido}>: {texto}\n", tag)
...
class ChatClienteGUI(ctk.CTk):
            ...
            try:
                ...
                while "\n" in buffer_dados:
                    ...
                    if tipo == "mensagem":
                        ...
                        self.after(0, lambda r=remetente, h=hora, t=texto: self.processar_msg_privada_recebida(r, h, r, t))
    ...
    def processar_msg_privada_recebida(self, usuario_janela, hora, remetente, texto):
        ...
        if janela:
            janela.exibir_mensagem(hora, remetente, texto)
